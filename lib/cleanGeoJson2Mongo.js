        ...
        if (path_stat.isFile()) {
            ...
            fs.readdirSync(path).forEach(function (file) {
                ...
                if (fs.statSync(path + '/' + file).isFile()) {
                    ...
                    // console.log('Resultado ' + patt_ini.test(JSON.stringify(converted)));
                    // console.log('Resultado Fin ' + patt_fin.test(JSON.stringify(converted)));
                    converted.features[0]["proccessed"] = false;
                    converted.features[0]["created_at"] = new Date();
                    converted.features[0]["updated_at"] = new Date();
                    var converted_string = JSON.stringify(converted);
...
exports.parseGeoJson = function () {
    var ret = "- ok -";

    path = '/DATA/PROYECTOS/DOMINICA/Requisitos/GEOJSON_SHAPES/landslide_5c_orig.geojson';
    var path_stat = fs.statSync(path);
    var fileconv = new DOMParser().parseFromString(fs.readFileSync(path, 'utf8'));
    //console.log(fileconv);
    //console.log(utm.convertLatLngToUtm(667588.9760351551231, 1727707.167285240953788));
    var utm = Utm.parse('667588.9760351551231 1727707.167285240953788');
    var latlon = utm.toLatLonE();
    console.log(latlon.toString('d', 5));
    if (path_stat.isFile()) {
